                                     ...
                                     ke_task_id_t const src_id)
{
    ...
    ASSERT_ERR(ancc_env != NULL);
    ASSERT_ERR(ancc_env->env[conidx] != NULL);
	NS_LOG_DEBUG("%s\r\n", __func__);
                                             ...
                                             ke_task_id_t const src_id)
{
   ...
   if (ancc_env->state[conidx] == ANCC_DISCOVERING)
 	{
 ...
 else if (ancc_env->state[conidx] == ANCC_BUSY)
 	{
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    uint8_t conidx = KE_IDX_GET(dest_id);
	NS_LOG_DEBUG("%s\r\n", __func__);
  if (ancc_env->state[conidx] == ANCC_IDLE)
    {
...
/// Specifies the ancc default message handlers
...
void ancc_task_init(struct ke_task_desc *p_task_desc)
{
    ...
    struct ancc_env_tag *p_ancc_env = PRF_ENV_GET(ANCC, ancc);

    p_task_desc->msg_handler_tab = ancc_msg_handler_tab;
    p_task_desc->msg_cnt         = ARRAY_LEN(ancc_msg_handler_tab);
    p_task_desc->state           = p_ancc_env->state;
    p_task_desc->idx_max         = ANCC_IDX_MAX;
}
