...
const struct attm_desc blps_att_db[BPS_IDX_NB] =
{
    ...
    [BPS_IDX_RACP_CHAR] = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // Record Access Control Point Characteristic Value
    [BPS_IDX_RACP_VAL] =
              {ATT_CHAR_REC_ACCESS_CTRL_PT, PERM(IND, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, BLPS_RACP_MAX_LEN},
    // Record Access Control Point Characteristic - Client Characteristic Configuration Descriptor
    [BPS_IDX_RACP_IND_CFG] = {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, 0},
};
        ...
        uint8_t sec_lvl, struct blps_db_cfg *p_params)
{
    ...
    //Set Configuration Flag Value
    if (GETB(p_params->prfl_cfg, BLPS_RACP_SUP))
    {
        cfg_flag |= BLPS_RACP_MASK;
    }
...
void blps_send_cmp_evt(struct blps_env_tag *p_blps_env, uint8_t conidx, uint8_t operation, uint8_t operation_code, uint8_t status)
{
    ...
    p_evt->status      = status;
    p_evt->operation_code = operation_code;
