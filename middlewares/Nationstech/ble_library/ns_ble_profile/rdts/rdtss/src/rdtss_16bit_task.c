...
void rdtss_16bit_init_ccc_values(const struct attm_desc *att_db, int max_nb_att)
{
    ...
    for (i = 1; i < max_nb_att; i++)
    {
        if( PERM_GET(att_db[i].perm, UUID_LEN) == PERM_UUID_16 &&
            (att_db[i].uuid) == ATT_DESC_CLIENT_CHAR_CFG
        )
        {
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    {
        NS_LOG_ERROR("status err:%d\r\n", status);
        return KE_MSG_SAVED;
    }             
    rdtss_16bit_env->operation = ke_param2msg(param);
    rdtss_16bit_env->cursor = conidx;
    ...
    if ( (ccc_values[conidx] & PRF_CLI_START_NTF)  || (ccc_values[conidx] & PRF_CLI_START_IND) )
    {
        struct gattc_send_evt_cmd *req;
        // Allocate the GATT notification message
        req = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
            KE_BUILD_ID(TASK_GATTC, conidx), rdtss_16bit_env->operation->dest_id, gattc_send_evt_cmd, app_req->length);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (ccc_values[conidx] & PRF_CLI_START_IND)
    {
        struct gattc_send_evt_cmd *req;
        // Allocate the GATT notification message
        req = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(state == RDTSS_16BIT_IDLE)
    {
        ...
        {
            if( PERM_GET(rdtss_16bit_env->db_cfg.att_tbl[att_idx].perm, UUID_LEN) == PERM_UUID_16 &&
