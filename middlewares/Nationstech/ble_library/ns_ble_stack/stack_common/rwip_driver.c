...
extern uint32_t OTPTrim_Read(uint32_t address, uint8_t* p_data, uint32_t byte_length);
...
#define MAX_SLEEP_DURATION_EXTERNAL_WAKEUP      0x7FFFFFF //(41943039ms)   0x7D00  //10s
...
#ifndef TRIM_RADIO_FREQUENCY_ENABLE
#define TRIM_RADIO_FREQUENCY_ENABLE   1
#endif

#if TRIM_RADIO_FREQUENCY_ENABLE
#define STANDARD_MCO_FREQ           8000000
...
static uint32_t actual_frequency = 0xffffffff;
static uint32_t modem_reg_bc,modem_reg_bd,modem_reg_be; 

uint32_t radio_read_trim_mco_freq(void)
{
    trim3_stored_t trim3_read ={0};
    //read radio trim frim otp3
    OTPTrim_Read(TRIM_STORE_ADDR3, (uint8_t*)&trim3_read, sizeof(trim3_stored_t));
    if(trim3_read.mark == TRIM_STORED_MARK)
    {
        return trim3_read.f_cal;
    }
    return 0; // without trim value stored 
}

bool radio_read_trim_mac_address(uint8_t *p_mac)
{
    trim3_stored_t trim3_read ={0};
    //read radio trim frim otp3
    OTPTrim_Read(TRIM_STORE_ADDR3, (uint8_t*)&trim3_read, sizeof(trim3_stored_t));
    ...
    if(trim3_read.mark == TRIM_STORED_MARK)
    {
        memcpy(p_mac,trim3_read.user_mac,6);
        return true;
    }
    return false; // without trim value stored 
}

/* call initial and after sleep,  */ 
void radio_freq_cal_from_trim(void)
{
    if(actual_frequency < 8008000 && actual_frequency > 7002000 )
    {
...
void radio_hse_mco_set(uint32_t mco_freq)
{
    int32_t     delta_fref;
    float       err;
    uint32_t    delta_k;
    
    actual_frequency = mco_freq;
    delta_fref =  STANDARD_MCO_FREQ - actual_frequency;
    err = (float)delta_fref / (float)actual_frequency;
...
#ifdef READ_HSE_MCO_FROM_TRIM_DEMO
/* demo function to read triming of HSE and set offset  */
void read_hse_freq_and_config_demo(void)
{
    uint32_t real_mco_freq; 
    real_mco_freq = radio_read_trim_mco_freq();
    radio_hse_mco_set(real_mco_freq);
    NS_LOG_DEBUG("Radio trim freq:%d\r\n",real_mco_freq);
}
#endif
#endif
/**
 ...
 ****************************************************************************************
 */
__STATIC __INLINE uint64_t rwip_lpcycles_2_hus(uint32_t lpcycles, uint32_t *error_corr)
{
    uint64_t res;
...
//    float  tmep = (float)g_lsi_1_syscle_cnt_value/1000.0;
		double  tmep = (double)g_lsi_1_syscle_cnt_value/1000.0;
    res = (((uint64_t)62 * lpcycles + res)*tmep);
...
//    lpcycles = hs_cnt * 10 * 1000/g_lsi_1_syscle_cnt_value;
    lpcycles = ((uint64_t)hs_cnt) * 10000 /g_lsi_1_syscle_cnt_value;
    lpcycles--;
...
__STATIC void rwip_wakeup(void)
{
    ...
    fintetime_correction = (HALF_SLOT_SIZE-1) - (dur_hus - ((uint64_t)dur_hslot)*HALF_SLOT_SIZE);
    // The correction values are then deduced from the sleep duration in us
    ip_clkncntcorr_pack(/*absdelta*/ 1, /*clkncntcorr*/ dur_hslot);
...
void llhwc_phy_prerx_flash(void)
{
    ...
    REG32(MODEM_BASE +0x139 *4) = 0x00845b02; //turn on vdd_ana
	...
	REG32(MODEM_BASE +0x14E *4) = 0x00055d12;
...
void rwip_slp_isr(void)
{
    ...
    if (irq_stat & IP_SLPINTSTAT_BIT)
    {
        ...
        g_sleep_status_flag = 0; //BB_ACTIVE
    }
}
