...
static void ns_ble_stack_vtor_init(void)
{
    ...
    #ifdef SLEEP_LP_TIMER_ENABLE
    p_IrqFun[RTC_IRQn]                  = (uint32_t)ns_RTC_IRQHandler;
...
static void app_set_adv_data(void)
{
    ...
    // Update advertising state
		if (app_env.adv_state != APP_ADV_STATE_STARTED)	//update adv data when adv started
...
static void app_set_scan_rsp_data(void)
{
    ...
    // Update advertising state
		if (app_env.adv_state != APP_ADV_STATE_STARTED)	//update rsp data when adv started
 ...
 **/
static void ns_ble_evn_init(void)
{
    NS_LOG_DEBUG("%s\r\n", __func__);
 ...
 * @brief set ble phy 
 **/
void ns_ble_phy_set(enum ble_phy_val phy)
{
...
void ns_ble_active_rssi(uint32_t interval)
{
    ...
    app_env.rssi_intv = interval;
		app_env.rssi_conidx = app_env.conidx;
    // request RSSI
...
bool ns_ble_adv_data_set(uint8_t* p_dat, uint16_t len)
{
...
bool ns_ble_scan_rsp_data_set(uint8_t* p_dat, uint16_t len)
{
...
bool ns_ble_ex_adv_data_set(uint8_t* p_dat, uint16_t len)
{
...
void ns_ble_start_init(uint8_t *addr, uint8_t addr_type)
{
    ...
    p_cmd->actv_idx = app_env.init_actv_idx;
		...
		if (app_env.wl_reconnect){
		p_cmd->u_param.init_param.type = GAPM_INIT_TYPE_AUTO_CONN_EST;
...
void ns_ble_stop_init(void)
{
    NS_LOG_DEBUG("%s,init_actv_idx = 0x%x\r\n", __func__,app_env.init_actv_idx);
    if (app_env.init_actv_idx != 0xFF)
    {
        struct gapm_activity_stop_cmd *p_cmd = KE_MSG_ALLOC(GAPM_ACTIVITY_STOP_CMD,
                                                              TASK_GAPM, TASK_APP,
...
/* Set white list */
void ns_ble_list_set_wl(struct gap_bdaddr *wl_addr,uint8_t wl_cnt)
{
...
void ns_ble_set_channel_map( uint8_t* newchmap)
{
    NS_LOG_DEBUG("%s\r\n", __func__);
 ...
 *         We can call the function: ns_ble_radio_calibration(2401950000, 2402000000);
 */
void ns_ble_radio_calibration(uint32_t fact_freq,uint32_t std_freq)
{
 ...
 * @note   
 */
void ns_ble_radio_power_set(rf_tx_power_t tx_pwr)
{
 ...
 * @note   
 */
void ns_peripheral_config(void)
{
    ...
    {
        /* IWDGRST flag set*/
        NS_LOG_WARNING("IWDGRST is set, reset from IWDG!!!\r\n");
    }
    /* Clear reset flags */
