                                             ...
                                             ke_task_id_t const src_id)
{
    ...
    else if(p_param->actv_type == GAPM_ACTV_TYPE_SCAN)
    {
        ...
        if( app_env.scan_actv_idx != 0xFF)
        {
            ...
            {
                scan_env.ble_scan_state_handler(SCAN_STATE_IDLE);
            }
        }
    }
    else if(p_param->actv_type == GAPM_ACTV_TYPE_INIT)
    ...
    switch(p_param->operation)
    {
        ...
        case (GAPM_RESET):
        {
            if(p_param->status == GAP_ERR_NO_ERROR)
            {
                ...
                if (((gap_env.mac_addr.addr[5] & 0xC0) == 0xC0) && (!(gap_env.dev_role&GAP_ROLE_CENTRAL))) //central can't set PRIV
                {
                    // Host privacy enabled by default
                    p_cmd->privacy_cfg |= GAPM_PRIV_CFG_PRIV_ADDR_BIT;
                }
                p_cmd->privacy_cfg |= GAPM_PRIV_CFG_PRIV_EN_BIT;
                p_cmd->renew_dur = 36000;//900;
                ...
                //init rand function
                struct gapm_gen_rand_nb_cmd *p_rand_cmd = KE_MSG_ALLOC(GAPM_GEN_RAND_NB_CMD,
                                                                TASK_GAPM, TASK_APP,
                                                                gapm_gen_rand_nb_cmd);
                p_rand_cmd->operation = GAPM_GEN_RAND_NB;
                ke_msg_send(p_rand_cmd);
            }
            else
            {
                ASSERT_ERR(0);
        ...
        case (GAPM_SET_DEV_CONFIG):
        {
            ...
            #if (BLE_APP_SEC)  
			struct gap_bdaddr bond_addr={0};
            if (ns_sec_get_bond_status()==true)
            {
                ns_bond_last_bonded_ral_info(&dev_info);
				ns_bond_last_bonded_addr(&bond_addr);
            }
			if ((ns_sec_get_bond_status()==true) && (dev_info.addr.addr_type == 0) && (bond_addr.addr_type != 0) )
			{
        ...
        case (GAPM_RESOLV_ADDR):
        {
            ...
            if(p_param->status != CO_ERROR_NO_ERROR)
            {
                //not match yet, resolv next addr
                ns_bond_resolv_addr_start(&app_env.peer_addr,app_env.next_svc--);
            }
            #endif
        } break;
        ...
        case (GAPM_STOP_ACTIVITY):      
        {
            ...
            {
                // Perform next operation
							if (!((p_param->operation == GAPM_DELETE_ACTIVITY) && (app_env.adv_state != APP_ADV_STATE_IDLE)))
                ns_ble_adv_fsm_next();
            }
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        {
            //set the addr solved task in next 5ms.
			if (p_param->peer_addr_type == 1)
            ke_msg_send_basic(GAPM_ADDR_SOLVED_IND,TASK_APP,TASK_APP);
        ...
        {
            ke_timer_clear(APP_INIT_TIMEOUT_EVT,TASK_APP);
        }
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    memset (&app_env.peer_irk.key,0,GAP_KEY_LEN);
    if(l2cm_env.con_tx_state)
    {
                                        ...
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    //init rand function
    uint32_t real_rand;
    memcpy(&real_rand, &p_param->randnb.nb[0], 4);
    srand(real_rand);
    return (KE_MSG_CONSUMED);
}
...
static int gapm_addr_solved_ind_handler(ke_msg_id_t const msgid, 
                                          ...
                                          ke_task_id_t const src_id)
{
    NS_LOG_DEBUG("%s,src:%d\r\n",__func__,src_id);
    if(src_id == TASK_APP) //ke msg
    {
        //start solved
        ns_bond_search_addr_irk(&(app_env.peer_addr),app_env.peer_addr_type);
    }
    else
    {
        memcpy(app_env.peer_irk.key,p_param->irk.key ,GAP_KEY_LEN);
        #if 0 //debug
        NS_LOG_DEBUG("This mac solved:%02X %02X %02X %02X %02X %02X \r\n",
                                              ...
                                              p_param->addr.addr[4],
                                              p_param->addr.addr[5]);
        NS_LOG_DEBUG("This irk:");
        for(uint8_t i = 0; i<GAP_KEY_LEN;i++)
        {
            NS_LOG_DEBUG(" %02X",p_param->irk.key[i]);
        }
        NS_LOG_DEBUG("\r\n");
        #endif
    }
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if(src_id != 0xff)
    {
        ...
        struct gapc_get_info_cmd* info_cmd = KE_MSG_ALLOC(GAPC_GET_INFO_CMD,
                KE_BUILD_ID(TASK_GAPC, app_env.rssi_conidx), TASK_APP,
                gapc_get_info_cmd);
...
/** 
 * @brief Handles adv repoer of scan result.
 *
 * @param[in] msgid     Id of the message received.
 * @param[in] p_param   Pointer to the parameters of the message.
 * @param[in] dest_id   ID of the receiving task instance
...
/** 
 * @brief Handles scan request event from GAPM. Need to config GAPM_ADV_PROP_SCAN_REQ_NTF_EN_BIT in adv create
 *
 * @param[in] msgid     Id of the message received.
 * @param[in] p_param   Pointer to the parameters of the message.
 * @param[in] dest_id   ID of the receiving task instance
 ...
 * @return If the message was consumed or not. 
 */
static int gapm_scan_request_ind_handler(ke_msg_id_t const msgid, struct gapm_scan_request_ind const *param,
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
     NS_LOG_DEBUG("\r\nReport %x, ", param->trans_addr.addr_type);
    for(int i = 0; i < 6; i++)
...
/** 
 * @brief Handles timeout of master establishing a connection, it will stop init a
 *
 * @param[in] msgid     Id of the message received.
 * @param[in] p_param   Pointer to the parameters of the message.
 * @param[in] dest_id   ID of the receiving task instance
 ...
 * @return If the message was consumed or not. 
 */
static int app_init_timeout_evt_handler(ke_msg_id_t const msgid, void *p_param,
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    NS_LOG_DEBUG("%s\r\n",__func__);
    ns_ble_stop_init();
    if(scan_env.ble_scan_state_handler != NULL)
    {
        scan_env.ble_scan_state_handler(SCAN_STATE_CONN_TIMEOUT);
    }
    return (KE_MSG_CONSUMED);
}
...
KE_MSG_HANDLER_TAB(app)
{
    ...
    {APP_INIT_TIMEOUT_EVT,      (ke_msg_func_t)app_init_timeout_evt_handler},
